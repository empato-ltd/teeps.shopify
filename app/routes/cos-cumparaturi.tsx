...
import {type CartLine, type Cart} from '@shopify/hydrogen/storefront-api-types';
...
const CartProduct = ({line}: {line: CartLine}) => {
  return (
      ...
      {cart?.totalQuantity ? (
        <div className="px-4 py-2">
          {cart?.lines?.nodes?.map((node, index) => (
            <CartProduct key={`${node.id}`} line={node as CartLine} />
          ))}
        </div>
      ) : (
